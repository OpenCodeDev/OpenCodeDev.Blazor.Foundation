...
{
    public partial class MenuDrilldown : ComponentBase, IDisposable
    {
        [Parameter]
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            if (firstRender)
            {
                if (AutoManaged)
                {
                    await JSRuntime.InvokeVoidAsync("DrilldownRegister", Id, DataOptions != null ? DataOptions.ToJSObjectString() : null);
